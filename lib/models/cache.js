...
exports.expireCache = function(region, access_token, context, milliseconds){
    // Write to the Redis cache.
    if (context.redisClient){
        let key = region + '_' + access_token;
        logger.debug("Setting the cache expiration for: " + key);
        context.redisClient.expire(key, milliseconds);
    }
    else{
        logger.debug("expireCache: Missing redisClient.");
    }
}
