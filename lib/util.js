...
exports.getRootUrl = function(req, context) {
    let rootUrl = req.protocol + "://" + req.hostname;
    if (context.config.port && context.config.port.length > 0) {
        rootUrl += ":" + context.config.port;
    }
    return rootUrl;
}
